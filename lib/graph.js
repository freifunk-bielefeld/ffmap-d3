...
var community_labels = { "gotham" : "Gotham City" };
...
function getCommunityLabel(id) {
    var label = community_labels[id.toLowerCase()];
...
function eui6_address(mac) {
  var prefix = ffmapConfig.prefix6;
  prefix = prefix.substring(0, prefix.length - 1); // hack: cut off last :
  var m = mac.replace(/:/g, "");
  /* convert to globally administered MAC address */
  m=(parseInt(mac.substring(0,2), 16) ^ 2).toString(16)+m.substring(2);
  return prefix+m.substr(0,4)+":"+m.substr(4,2)+"ff:fe"+m.substr(6,2)+":"+m.substr(8,4);
}
...
function show_node_info(d) {
  ...
  if (d.contact) {
    ...
    nodeinfo.append("p")
            ...
            // graph when the user navigates back.
            .attr("target","_blank")
            .attr("href", "http://["+eui6_address(d.id)+"]/")
            .text("Statusseite");
}
...
function reload() {
  ...
  function handler(json) {
    ...
    for(i in communities) {
        ...
        if(label == community.name) continue;
