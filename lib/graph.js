...
function reload() {
  ...
  function handler(json) {
    ...
    var all_nodecount = 0;
    var all_clientcount = 0;
    var all_gwcount = 0;

    //collect comunities
    var objs = data.nodes.reduce(function(a, d) {
        var obj = a[d.community];
        if(obj) {
            obj.nodecount += d.flags.online;
        ...
        all_nodecount += d.flags.online;
        all_clientcount += d.clientcount;
        all_gwcount += (d.flags.gateway && d.flags.online);
        return a;
    }, {});
	...
	if(communities.length != 1) {
		...
		meshstats.append("p").text(all_nodecount + " Knoten / " + all_clientcount + " Clients");
		meshstats.append("p").text(all_gwcount + " Gateways / " + communities.length +  " Communities");
	}
    ...
    for(i in communities) {
        ...
        // skip unknown communities
        if(label == community.name) continue;

		meshstats.append("h2").append("a")
                  .attr("href", "http://"+community.name+".freifunk.net")
                  .text(function(d) {
                    return label;
                  })
        meshstats.append("p").text(community.nodecount + " Knoten / " + community.clientcount + " Clients / " + community.gwcount + " GWs");
    }
