...
function update(data) {
    ...
    var links = data.links.filter(function(d) {
        return ('geo' in d.source) && ('geo' in d.target) && d.type != "vpn"
    })
    ...
    var border = 0.0
    var bounds = [
        [t[0][0] - border, t[1][0] - border],
        [t[0][1] + border, t[1][1] + border]
    ]
    ...
    nodes_svg = g.select(".nodes").selectAll(".node").data(nodes, function(d) {
        return d.id
    })
    var links_svg = g.select(".links").selectAll(".link").data(links, function(d) {
        return d.id
    })
    ...
    function reset() {
        ...
        nodes_svg.attr("transform", function(d) {
            return "translate(" + project(d.geo).join(",") + ")"
        })

        links_svg.attr("x1", function(d) {
                return project(d.source.geo)[0]
            })
            .attr("y1", function(d) {
                return project(d.source.geo)[1]
            })
            .attr("x2", function(d) {
                return project(d.target.geo)[0]
            })
            .attr("y2", function(d) {
                return project(d.target.geo)[1]
            })
